...
# -*- coding: utf-8 -*-
"""
⚽ Half-Time Football Match Predictor - Flask Application
17 Leagues, Separate Models for Half-Time Results
"""
...
# Load all Half-Time models and stats
LEAGUES_HT = {}
models_folder = 'models_ht'
stats_folder = 'stats_ht'

print("=" * 80)
print("⚽ Loading Half-Time Models and Statistics...")
print("=" * 80)

# Get all model files
if os.path.exists(models_folder):
    for model_file in os.listdir(models_folder):
        if model_file.startswith('model_') and model_file.endswith('_HT.pkl'):
            ...
            try:
                ...
                home_stats_file = f'{stats_folder}/home_stats_{league}_HT_2025.csv'
                away_stats_file = f'{stats_folder}/away_stats_{league}_HT_2025.csv'
                ...
                if os.path.exists(home_stats_file) and os.path.exists(away_stats_file):
                    ...
                    print(f"✅ {league}: HT Model + Stats loaded")
            except Exception as e:
                print(f"❌ {league}: Error - {str(e)}")
else:
    print(f"⚠️  Models folder '{models_folder}' not found")
    print("   Please make sure models_ht/ exists in the working directory")

print(f"\n🎉 Total HT leagues loaded: {len(LEAGUES_HT)}\n")
...
def index():
    ...
    if not LEAGUES_HT:
        ...
        <h1>⚠️ Half-Time Predictor - Configuration Error</h1>
        <p>No models loaded. Please ensure:</p>
        <ul>
            <li>models_ht/ folder exists with .pkl files</li>
            <li>stats_ht/ folder exists with .csv files</li>
            <li>Both are in the application directory</li>
        </ul>
        </body>
        </html>
        """
    
    leagues_list = sorted(list(LEAGUES_HT.keys()))
    leagues_info = [(league, LEAGUE_NAMES.get(league, league)) for league in leagues_list]
    return render_template('index_ht.html', leagues=leagues_info)
...
def predict_ht(league):
    """Predict Half-Time result for selected league"""
    try:
        if league not in LEAGUES_HT:
            return jsonify({'error': 'League not found'}), 400
        ...
        # Model classes: ['A', 'D', 'H']
        prob_away = float(pred_proba[0][0]) * 100
        prob_draw = float(pred_proba[0][1]) * 100
        prob_home = float(pred_proba[0][2]) * 100
...
def health():
    ...
    return jsonify({
        'status': 'ok',
        'leagues_loaded': len(LEAGUES_HT),
        'prediction_type': 'Half-Time'
    })

if __name__ == '__main__':
    print("=" * 80)
    print("🚀 Starting Half-Time Football Predictor...")
    print("=" * 80)
    print(f"📱 Open: http://localhost:5000")
    print(f"⚽ Prediction Type: Half-Time Results")
    print(f"🎯 Leagues Available: {len(LEAGUES_HT)}")
    if len(LEAGUES_HT) > 0:
        print(f"📊 Leagues: {', '.join(sorted(LEAGUES_HT.keys()))}")
    print("=" * 80 + "\n")
